...
#Region "Form Event"
    ...
    Private Sub frm部门详细_KeyDown(sender As Object, e As KeyEventArgs) Handles Me.KeyDown
        Try
    ...
    Public Sub 部门查询_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
    ...
    Public Sub 部门查询_FormClosing(ByVal sender As Object, ByVal e As System.Windows.Forms.FormClosingEventArgs) Handles Me.FormClosing
...
#Region "Button Event"
    ...
    Private Sub btnF2编辑_Click(sender As Object, e As EventArgs) Handles btnF2编辑.Click
        ...
        Try
            ...
            If objFrm.ReturnType = Constant.ENU_SEARCH.SearchMOk Then
                ...
                txt部门编号.Text = objFrm.CD

                Dim dtTemp As DataTable = Utility.GetViewByKey("部门编号", objFrm.CD, "M_部门明细")

                If dtTemp.Rows.Count > 0 Then
                    txt部门名称.Text = dtTemp.Rows(0).Item("部门名称").ToString
                    'cbo部门类别.Value = dtTemp.Rows(0).Item("部门类别").ToString
                End If
    ...
    Private Sub btnF4删除_Click(sender As Object, e As EventArgs) Handles btnF4删除.Click
        ...
        Dim objM_部门详细T As M_部门详细T
        Dim objBll As Bll部门详细
        ...
        Try
            ...
            objM_部门详细T = New M_部门详细T
            objBll = New Bll部门详细

            objM_部门详细T.部门编号 = txt部门编号.Text

            objBll.DoDelete(objM_部门详细T)
...
#Region "Private Event"
    ...
    Public Sub SetCombox()
        ...
        'Utility.SetCombo(cbo部门类别, "区分", "名称", "VM部门类别", 1, "", "CAST(区分 AS INT)")
    ...
    Private Function CheckInput() As Boolean
        ...
        If txt部门编号.Text.Trim = String.Empty Then
            CommonMsg.showMsg(Me.Text, Constant.ENU_MSGID.MSGID13, "部门编号", "部门编号")
            txt部门编号.Focus()
            Return False
        End If
        ...
        If txt部门名称.Text.Trim = String.Empty Then
            CommonMsg.showMsg(Me.Text, Constant.ENU_MSGID.MSGID13, "部门名称", "部门名称")
            txt部门名称.Focus()
            Return False
        End If

        ''部门类别
        'If cbo部门类别.Text.Trim = String.Empty Then
        '    CommonMsg.showMsg(Me.Text, Constant.ENU_MSGID.MSGID13, "部门类别", "部门类别")
        '    cbo部门类别.Focus()
        '    Return False
        'End If
    ...
    Private Function SaveData() As Boolean
        ...
        Dim objM_部门详细T As New M_部门详细T
        Dim objBll As New Bll部门详细

        objM_部门详细T.部门编号 = txt部门编号.Text
        objM_部门详细T.部门名称 = txt部门名称.Text
        'objM_部门详细T.部门类别 = cbo部门类别.Value
        objM_部门详细T.使用状态 = 0
        ...
        If Z_FormMode = Constant.ENU_FORMMODE.ModeNew Then
            ...
            '保存到DB
            If objBll.DoInsert(objM_部门详细T) = Constant.ENU_RETURNTYPE.KeyRepeat Then
    ...
    Public Sub SetPermission()
        ...
        '        'F4削除ボタン
        '        btnF4.Enabled = Utility.GetPermission(Constant.cgStrPerm_部门查询_削除, Me.Z_DtPermission)

        '        'F6新規ボタン
        '        btnF6.Enabled = Utility.GetPermission(Constant.cgStrPerm_部门查询_新規, Me.Z_DtPermission)
