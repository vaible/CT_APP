...
#Region "Form Event"
    ...
    Private Sub frm生产线详细_KeyDown(sender As Object, e As KeyEventArgs) Handles Me.KeyDown
        Try
    ...
    Public Sub 生产线查询_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
    ...
    Public Sub 生产线查询_FormClosing(ByVal sender As Object, ByVal e As System.Windows.Forms.FormClosingEventArgs) Handles Me.FormClosing
...
#Region "Button Event"
    ...
    Private Sub btnF2编辑_Click(sender As Object, e As EventArgs) Handles btnF2编辑.Click
        ...
        Try
            ...
            If objFrm.ReturnType = Constant.ENU_SEARCH.SearchMOk Then
                ...
                txt生产线编号.ReadOnly = True

                txt生产线编号.Text = objFrm.CD

                Dim dtTemp As DataTable = Utility.GetViewByKey("生产线编号", objFrm.CD, "M_生产线详细")

                If dtTemp.Rows.Count > 0 Then
                    txt生产线名称.Text = dtTemp.Rows(0).Item("生产线名称").ToString
                    'cbo生产线类别.Value = dtTemp.Rows(0).Item("生产线类别").ToString
                End If
    ...
    Private Sub btnF4删除_Click(sender As Object, e As EventArgs) Handles btnF4删除.Click
        ...
        Dim objM_生产线详细T As M_生产线详细T
        Dim objBll As Bll生产线详细
        ...
        Try
            ...
            objM_生产线详细T = New M_生产线详细T
            objBll = New Bll生产线详细

            objM_生产线详细T.生产线编号 = txt生产线编号.Text

            objBll.DoDelete(objM_生产线详细T)
...
#Region "Private Event"
    ...
    Public Sub SetCombox()
        ...
        'Utility.SetCombo(cbo生产线类别, "区分", "名称", "VM生产线类别", 1, "", "CAST(区分 AS INT)")
    ...
    Public Sub InitForm()
        ...
        '画面控件内容清空
        txt生产线编号.Text = ""
        txt生产线编号.ReadOnly = False

        txt生产线名称.Text = ""
        'cbo生产线类别.Value = Nothing
    ...
    Private Function CheckInput() As Boolean
        ...
        '必填项检查
        '生产线编号
        If txt生产线编号.Text.Trim = String.Empty Then
            CommonMsg.showMsg(Me.Text, Constant.ENU_MSGID.MSGID13, "生产线编号", "生产线编号")
            txt生产线编号.Focus()
            Return False
        End If

        '生产线名
        If txt生产线名称.Text.Trim = String.Empty Then
            CommonMsg.showMsg(Me.Text, Constant.ENU_MSGID.MSGID13, "生产线名", "生产线名")
            txt生产线名称.Focus()
            Return False
        End If

        ''生产线类别
        'If cbo生产线类别.Text.Trim = String.Empty Then
        '    CommonMsg.showMsg(Me.Text, Constant.ENU_MSGID.MSGID13, "生产线类别", "生产线类别")
        '    cbo生产线类别.Focus()
        '    Return False
        'End If
    ...
    Private Function SaveData() As Boolean
        ...
        Dim objM_生产线详细T As New M_生产线详细T
        Dim objBll As New Bll生产线详细

        objM_生产线详细T.生产线编号 = txt生产线编号.Text
        objM_生产线详细T.生产线名称 = txt生产线名称.Text
        'objM_生产线详细T.生产线类别 = cbo生产线类别.Value
        objM_生产线详细T.使用状态 = 0
        ...
        If Z_FormMode = Constant.ENU_FORMMODE.ModeNew Then
            ...
            '保存到DB
            If objBll.DoInsert(objM_生产线详细T) = Constant.ENU_RETURNTYPE.KeyRepeat Then
    ...
    Public Sub SetPermission()
        ...
        '        'F4削除ボタン
        '        btnF4.Enabled = Utility.GetPermission(Constant.cgStrPerm_生产线查询_削除, Me.Z_DtPermission)

        '        'F6新規ボタン
        '        btnF6.Enabled = Utility.GetPermission(Constant.cgStrPerm_生产线查询_新規, Me.Z_DtPermission)
